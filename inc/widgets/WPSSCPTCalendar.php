...
add_action( 'widgets_init', 'wpss_widget_cpt_calendar' );
function wpss_widget_cpt_calendar() {
    register_widget( 'WPSSCPTCalendar' );
}

class WPSSCPTCalendar extends WP_Widget {

    public function __construct() {
        $options = array(
            'classname'   => 'widget_calendar',
            'description' => __( 'A calendar for your custom post types.', 'wpss' )
        );

        parent::__construct( 'wpss-post-type-calendar', __( '(WPSS) Calendar (Custom Post Type)', 'wpss' ), $options );
    ...
    public function widget( $args, $instance ) {
        ...
        $title    = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Calendar', 'wpss' ) : $instance['title'], $instance, $this->id_base );
    ...
    public function form( $instance ) {
        ...
        <p>
            <label for="<?php echo $this->get_field_id( 'posttype' ); ?>"><?php _e( 'Post Type:', 'wpss' ); ?></label>
            <select name="<?php echo $this->get_field_name( 'posttype' ); ?>" id="<?php echo $this->get_field_id( 'posttype' ); ?>">
                ...
                foreach ( $post_types as $post_type => $value ) {
                    ...
                    ?>
                    <option value="<?php echo esc_attr( $post_type ); ?>"<?php selected( $post_type, $posttype ); ?>><?php _e( $value->label, 'wpss' ); ?></option>
                <?php } ?>
            </select>
        </p>
        <?php
    ...
    public function get_custom_post_type_calendar( $posttype, $initial = true, $echo = true ) {
        ...
        if ( $previous ) {
            $calendar_output .= "\n\t\t" . '<td colspan="3" id="prev"><a href="' . $this->get_custom_post_type_month_link( $posttype, $previous->year, $previous->month ) . '/01">&laquo; ' . $wp_locale->get_month_abbrev( $wp_locale->get_month( $previous->month ) ) . '</a></td>';
        } else {
        ...
        if ( $next ) {
            $calendar_output .= "\n\t\t" . '<td colspan="3" id="next"><a href="' . $this->get_custom_post_type_month_link( $posttype, $next->year, $next->month ) . '/01">' . $wp_locale->get_month_abbrev( $wp_locale->get_month( $next->month ) ) . ' &raquo;</a></td>';
        } else {
